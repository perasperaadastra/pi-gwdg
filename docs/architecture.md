...
| `/gwdg-models` | Inline | None | Lists all GWDG models grouped by capability (text / vision / embeddings) with tree formatting, plus a right-aligned `demand` column whose header sits on the title line directly above the values (the values themselves carry no label, so the two column starts must stay in sync). Demand is a live load value, so it is fetched from `/models` on every invocation (`fetchModelDemand`) rather than read from the model cache; if the fetch fails (no API key, offline) the column is dropped and the header says `demand unavailable` |
